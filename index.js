...
program
    ...
    .option("--full-scan", "ignore state and scan all pages")
...
const stateFile = path.join("state", "article-crawler-state.json");
...
async function main() {
    ...
    if (opts.article) {
        articles = [{ url: opts.article, lastmod: "" }];
    } else {
        const state = loadState();
        const allArticles = await loadSitemap(opts.sitemap);

        articles = allArticles;

        if (!opts.fullScan && state.last_run) {
            const lastRun = new Date(state.last_run);

            articles = articles.filter((article) => {
                if (!article.lastmod) return true;
                return new Date(article.lastmod) > lastRun;
            });

            console.log(`Last run: ${state.last_run}`);
            console.log(`New/updated articles: ${articles.length}`);
        }
